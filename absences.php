...
<table class="table table-striped">
  ...
  <tbody>
  <?php
  $result2 = mysqli_query($con,"SELECT * from absence where studentid =1 order by absencedate") or die(mysqli_error($con));
	$loop2 = 0;
    ...
    <tr>
      <th scope="row"><?php echo $loop2; ?></th>
	        <td><?php echo date("d/m/Y", strtotime($row2['absencedate'])); ?></td>

      <td><?php echo $row2['hours']; ?></td>
	   <td><?php echo $row2['reason']; ?></td>
	   	   <td><?php 
		   if ($row2['communication']==0)
		   {
			$x = '-';
		   }
		   else
		   {
			   $x = $row2['communicationperson'];
		   }
